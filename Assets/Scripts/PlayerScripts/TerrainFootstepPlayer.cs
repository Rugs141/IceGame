...
[RequireComponent(typeof(CharacterController), typeof(AudioSource))] // Requires two components on this game object | character Controller & audiosource
public class TerrainFootstepPlayer : MonoBehaviour
{
    [SerializeField]
    private LayerMask FloorLayer; // the layer the object will need to produce the sound
    [SerializeField]
    private TextureSound[] TextureSounds; // the array of sounds for the texture
    [SerializeField]
    private bool BlendTerrainSounds; // bool to blend terrain sounds
    ...
    private IEnumerator CheckGround()
    {
        while (true)
        {
            ...
            if (Controller.isGrounded && Controller.velocity != Vector3.zero &&
                Physics.Raycast(transform.position,Vector3.down,out RaycastHit hit,1f,FloorLayer)
                )
            {
